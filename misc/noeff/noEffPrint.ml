...
let pp_header ppf =
  print ppf
    ...
    \    | Call (eff, arg, k) -> Call (eff, arg, fun x -> coer_computation \
     coer (k x)) \n\
    \      \n\
...
let rec pp_term noEff_term ppf =
  ...
  | Apply (t1, t2) -> (
      match t1 with
      | Cast (t, CoerArrow (c1, c2)) ->
          print ppf "%t" (pp_term (Cast (Apply (t, Cast (t2, c1)), c2)))
      | Cast (t, HandToFun (c1, c2)) ->
          print ppf "%t"
            (pp_term (Cast (Handle (Return (Cast (t2, c1)), t), c2)))
      | _ -> print ppf "@[<hov 2>(%t) @,(%t)@]" (pp_term t1) (pp_term t2))
  | BigLambdaTy (_, t) -> pp_term t ppf
  | ApplyTy (Cast (t, Forall (a, c)), ty) ->
      print ppf "%t"
        (pp_term
           (Cast
              ( ApplyTy (t, ty),
                NoEffSubstitute.substitute_tyvar_in_coercion a ty c )))
  | ApplyTy (t, _) -> pp_term t ppf
  | BigLambdaCoerVar (tycp, tyc, t) -> print ppf "%t" (pp_term t)
  | ApplyTyCoercion (BigLambdaCoerVar (cp, pi, t), c1) ->
      print ppf "%t"
        (pp_term (NoEffSubstitute.substitute_coer_var_in_term cp c1 t))
  | ApplyTyCoercion (Cast (t, CoerQualification (pi, c1)), c2) ->
      print ppf "%t" (pp_term (Cast (ApplyTyCoercion (t, c2), c1)))
  | ApplyTyCoercion (t, c) ->
      print ppf "@[<hov 2>(%t) @,(%t)@]" (pp_term t) (pp_coercion c)
  | Cast (t, c) ->
  ...
  | Handle
      (NoEffSyntax.Call (e, t1, (pat, ty, t)), Cast (t2, FunToHand (c1, c2))) ->
      print ppf "%t"
        (pp_term
           (NoEffSyntax.Call
              (e, t1, (pat, ty, Handle (t, Cast (t2, FunToHand (c1, c2)))))))
  | Handle (Return t1, Cast (t2, FunToHand (c1, c2))) ->
      print ppf "%t" (pp_term (Cast (Apply (t2, Cast (t1, c1)), c2)))
  | Handle (t1, t2) -> (
      match t2 with
      | Cast (t, CoerHandler (c1, c2)) ->
          print ppf "%t" (pp_term (Cast (Handle (Cast (t1, c1), t), c2)))
      | _ -> print ppf "@[<hov 2>(%t) @,(%t)@]" (pp_term t1) (pp_term t2))
...
let pp_external name symbol_name translation ppf =
  ...
  | NoEffExternal.Unknown ->
      print ppf "let %t = failwith \"Unknown external symbol %s.\"@."
        (pp_variable name) symbol_name
  | NoEffExternal.Exists t -> print ppf "let %t = %s@." (pp_variable name) t
...
let pp_cmd cmd ppf =
  ...
  | External (x, ty, f) -> (
      ...
      | Some (NoEffExternal.Exists s as known) -> pp_external x f known ppf)
