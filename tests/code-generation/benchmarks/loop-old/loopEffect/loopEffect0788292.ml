...
let _incr_handler_216 comp =
  handler
    {
      value_clause =
        (fun _y_222  ->
           value (fun _x_130  -> value ((fun _x_223  -> _x_223) _x_130)));
...
let _state_handler_224 comp =
  handler
    {
      value_clause =
        (fun _y_231  ->
           value (fun _x_131  -> value ((fun _x_232  -> _x_232) _x_131)));
      effect_clauses = fun (type a) -> fun (type b) ->
        fun (x : (a,b) effect)  ->
          (match x with
           | Effect_Get  ->
...
let rec _loop_pure_233 _n_234 _a_235 =
  if _n_234 = 0 then _a_235 else _loop_pure_233 (_n_234 - 1) (_a_235 + 1) 
let rec _loop_incr_243 _n_244 =
  if _n_244 = 0
  then value ()
  else call Effect_Incr () (fun _result_30  -> _loop_incr_243 (_n_244 - 1)) 
...
let _incr_261 _n_262 =
  (let rec _newvar_46 _n_244 =
     if _n_244 = 0
     then fun _x_51  -> _x_51
     else (fun _x_67  -> _newvar_46 (_n_244 - 1) (_x_67 + 1))  in
   _newvar_46 _n_262) 0
  
let _state_264 _n_265 =
  (let rec _newvar_77 _n_250 =
     if _n_250 = 0
     then fun _x_89  -> _x_89
     else (fun _s_127  -> _newvar_77 (_n_250 - 1) (_s_127 + 1))  in
   _newvar_77 _n_265) 0
