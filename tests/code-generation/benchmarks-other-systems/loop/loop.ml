...
commit SHA: 0d12f8c0b5f1393ff85e7d5d5b9743c6dd5a7bfe
=== BEGIN SOURCE ===
...
let rec _fold_left_77 _f_78 _a_79 _gen_function_80 =
  ...
  | _y_81::_ys_82 ->
      (_f_78 _a_79) >>
        ((fun _gen_bind_84  ->
            (_gen_bind_84 _y_81) >>
              (fun _a_83  -> _fold_left_77 _f_78 _a_83 _ys_82)))
...
let _mem_117 _x_118 =
  _exists_110
    (fun _lift_fun_57  ->
       value ((fun _x'_119  -> _x_118 = _x'_119) _lift_fun_57))
...
let _var_219 = (+)
type (_,_) effect +=
  | Effect_Fail: (unit,bool) effect
let rec _loop_latent_220 _n_221 =
  if _n_221 = 0
  then call Effect_Fail () (fun _result_29  -> value _result_29)
  else
    if _n_221 < 0
    then call Effect_Fail () (fun _result_31  -> value _result_31)
    else _loop_latent_220 (_n_221 - 1)
let _latent_handler_228 comp =
  handler
    {
      value_clause = (fun _y_230  -> value _y_230);
      effect_clauses = fun (type a) -> fun (type b) ->
        fun (x : (a,b) effect)  ->
          (match x with
           | Effect_Fail  ->
               (fun (() : unit)  -> fun (_k_229 : bool -> _)  -> _k_229 true)
           | eff' -> (fun arg  -> fun k  -> Call (eff', arg, k)) : a ->
                                                                    (b -> _)
                                                                    ->
                                                                    _)
    } comp
let _test_latent_231 _n_232 = _loop_latent_220 _n_232
let _ =
  let rec _loop_latent_36 _n_221 =
    if _n_221 = 0
    then true
    else if _n_221 < 0 then true else _loop_latent_36 (_n_221 - 1) in
  _loop_latent_36 10000
