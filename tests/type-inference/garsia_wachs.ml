...
let phase1 l =
  let rec extract before = function
    | [ (t, _) ] -> t
    | [ (t1, w1); (t2, w2) ] -> insert [] (Node (t1, t2), w1 + w2) before
    | (t1, w1) :: ((t2, w2) :: ((_, w3) :: _ as after) as r) ->
        if w1 <= w3 then insert after (Node (t1, t2), w1 + w2) before
        else extract ((t1, w1) :: before) r
  and insert after ((_, wt) as t) = function
    ...
    | ((_, wj_1) as tj_1) :: before ->
        ...
        else insert (tj_1 :: after) t before
  in
  extract [] l
