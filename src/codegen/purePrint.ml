...
and dirty_shape = ValueS of ty_shape | Computation of ty_shape
...
and print_dirty_shape ?(max_level= 1000) shape ppf =
  let print ?at_level = Print.print ~max_level ?at_level ppf in
  match shape with
  | ValueS shape -> print_ty_shape ~max_level shape ppf
  | Computation shape ->
...
let rec shape_of_ty st = function
  ...
  | Type.Apply (ty_name, args)
    when Tctx.transparent ~loc:Location.unknown ty_name -> (
    match Tctx.ty_apply ~loc:Location.unknown ty_name args with
  ...
  | Type.Handler (dirty1, dirty2) ->
      Handler (shape_of_dirty st dirty1, shape_of_dirty st dirty2)
  | ty ->
      Print.debug "Don't know the shape of %t" (Type.print_ty ty) ;
      assert false
...
and shape_of_dirty ((ctx, constraints) as st) (ty, drt) =
  ...
  else Computation (shape_of_ty st ty)
...
let rec simplify_ty_conversion = function
  ...
  | Tuple convs ->
      ...
      if List.for_all (fun conv -> conv = TyIdentity) convs then TyIdentity
      else Tuple convs
  | Lift (conv1, conv2) -> (
    match (simplify_ty_conversion conv1, simplify_dirty_conversion conv2) with
    | TyIdentity, DirtyIdentity -> TyIdentity
    | conv1, conv2 -> Lift (conv1, conv2) )
  | LiftHandler (conv1, conv2) ->
    match
      (simplify_dirty_conversion conv1, simplify_dirty_conversion conv2)
    with
...
and simplify_dirty_conversion = function
  ...
  | ConvertValues conv -> (
    match simplify_ty_conversion conv with
...
let rec ty_shape_conversion = function
  ...
  | Tuple shapes1, Tuple shapes2 ->
      Tuple
        (List.map2
           (fun shape1 shape2 -> ty_shape_conversion (shape1, shape2))
           shapes1 shapes2)
  | shape1, shape2 -> TyIdentity
...
let rec print_ty_conversion ?(max_level= 100000) conv term ppf =
  let print ?at_level = Print.print ~max_level ?at_level ppf in
  match conv with
  | TyIdentity -> (term max_level) ppf
  | Lift (conv1, conv2) ->
      let x = Typed.Variable.fresh "lift_fun" in
      print ~at_level:2 "(* both *)fun %t -> %t" (print_variable x)
        (print_dirty_conversion conv2 (fun m ppf ->
             Print.print ~max_level:m ppf "%t %t" (term 1)
               (print_ty_conversion ~max_level:0 conv1 (fun _ ->
                    print_variable x )) ))
  | LiftHandler (conv1, conv2) ->
      ...
      print ~at_level:2 "(* both *)fun %t -> %t" (print_variable x)
        (print_dirty_conversion conv2 (fun m ppf ->
             Print.print ~max_level:m ppf "%t %t" (term 1)
               (print_dirty_conversion ~max_level:0 conv1 (fun _ ->
                    print_variable x )) ))
  | Tuple convs ->
      let xs =
        List.mapi
          (fun i conv ->
            (Typed.Variable.fresh ("comp" ^ string_of_int i), conv) )
          convs
      in
      print ~at_level:2 "let (%t) = %t in (%t)"
        ...
        (Print.sequence ", "
           (fun (x, conv) ->
             print_ty_conversion conv (fun _ -> print_variable x) )
           xs)
...
and print_dirty_conversion ?(max_level= 1000) conv term ppf =
  let print ?at_level = Print.print ~max_level ?at_level ppf in
  match conv with
  | DirtyIdentity -> term max_level ppf
  | ConvertValues conv -> print_ty_conversion ~max_level conv term ppf
  | ConvertComps conv ->
...
let rec print_expression ?max_level ?expected_shape e ppf =
  ...
  Print.debug ~loc:e.Typed.location
    "Printing expression %t@. Type is %t@. Shape is %t"
...
and print_converted_expression ?max_level ppf = function
  | Tuple convs, {term= Typed.Tuple lst} ->
      Print.print ?max_level ppf "(%t)"
        (Print.sequence ", "
           (fun (conv, e) ppf -> print_converted_expression ppf (conv, e))
           (List.combine convs lst))
  | conv, e ->
      print_ty_conversion ?max_level conv
        (fun m -> print_expression' ~max_level:m e)
        ppf
...
and print_expression' ?max_level e ppf =
  ...
  | Typed.Var x -> print "%t" (print_variable x)
  | Typed.BuiltIn (s, _) -> print "%s" s
  | Typed.Const c -> print "%t" (Const.print c)
  | Typed.Tuple lst -> Print.tuple print_expression lst ppf
  | Typed.Record lst -> Print.record print_expression lst ppf
  | Typed.Variant (lbl, None) -> print "%s" lbl
  ...
  | Typed.Handler h ->
      ...
      print ~at_level:2 "fun comp -> handler %t comp"
        (print_handler ~expected_shape h)
  | Typed.Effect eff -> print ~at_level:2 "effect %t" (print_effect eff)


and print_computation ?max_level ?expected_shape c ppf =
  let shp = shape_of_dirty_scheme c.Typed.scheme in
  Print.debug ~loc:c.Typed.location
    "Printing computation %t@. Type is %t@. Shape is %t"
...
and print_converted_computation ?max_level ppf = function
  | conv, c ->
      print_dirty_conversion ?max_level conv
        (fun m -> print_computation' ~max_level:m c)
        ppf
...
and print_effect_clauses ~expected_shape eff_clauses ppf =
  ...
  | (((_, (t1, t2)) as eff), {Typed.term= p1, p2, c}) :: cases ->
      print ~at_level:1
        ...
        (print_effect eff) (print_pattern p1) (print_type t1)
...
and print_abstraction ~expected_shape {Typed.term= p, c} ppf =
  Format.fprintf ppf "%t ->@;<1 2> %t" (print_pattern p)
    (print_computation ~expected_shape c)
...
and print_let_rec_abstraction (x, a) ppf =
  ...
  Format.fprintf ppf "%t = fun %t" (print_variable x)
    (print_abstraction ~expected_shape a)
...
let print_command (cmd, _) ppf =
  ...
  | Typed.DefEffect (eff, (ty1, ty2)) ->
      Print.print ppf "type (_, _) effect += %t : (%t, %t) effect"
        (print_effect eff) (print_type ty1) (print_type ty2)
  | Typed.Computation c -> print_computation c ppf
  | Typed.TopLet (defs, _) ->
      Print.print ppf "let %t"
        (Print.sequence "\nand\n" print_top_let_abstraction defs)
  | Typed.TopLetRec (defs, _) ->
      Print.print ppf "let rec %t"
        (Print.sequence "\nand\n" print_let_rec_abstraction defs)
  | Typed.Use fn -> Print.print ppf "#use %S" (compiled_filename fn)
  | Typed.External (x, ty, f) ->
      Print.print ppf "let %t = ( %s )" (print_variable x) f
  | Typed.Tydef tydefs -> print_tydefs tydefs ppf
  | Typed.Reset ->
