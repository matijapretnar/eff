...
let rec print_type ?max_level ty ppf =
  ...
  match ty with
  | Type.Apply ("empty", _) -> print "unit"
  | Type.Apply (ty_name, args) ->
      print ~at_level:1 "%t %s" (print_args args) ty_name
  | Type.TyParam p -> print "%t" (Params.print_ty_param p)
  | Type.Basic t -> print "(%s)" t
  | Type.Tuple tys ->
      print ~at_level:1 "(%t)" (Print.sequence "*" print_type tys)
  | Type.Arrow (ty, drty) ->
      print ~at_level:2 "(%t -> %t)"
        (print_type ~max_level:1 ty)
        (print_dirty_type drty)
  | Type.Handler {value= drty1; finally= drty2} ->
      print ~at_level:2 "(%t -> %t)" (print_dirty_type drty1)
        (print_dirty_type drty2)
...
let rec print_params params ppf =
  ...
  | tys ->
      Format.fprintf ppf "(%t)"
        (Print.sequence "," Params.print_type_param tys)


let print_tydef_body ty_def ppf =
  match ty_def with
  | Tctx.Record flds ->
      let print_field (fld, ty) ppf =
        Format.fprintf ppf "%s: %t" fld (print_type ty)
      in
  ...
  | Tctx.Sum variants ->
      let print_variant (lbl, ty) ppf =
        ...
        | Some ty ->
            Format.fprintf ppf "%s of %t" lbl (print_type ~max_level:0 ty)
      in
      Format.fprintf ppf "@[<hov>%t@]"
        (Print.sequence "|" print_variant variants)
  | Tctx.Inline ty -> print_type ty ppf


let print_tydef (name, (params, body)) ppf =
  Format.fprintf ppf "%t %s = %t" (print_params params) name
    (print_tydef_body body)
...
let print_effect_region (eff, region) ppf =
  Print.print ppf "Effect_%s -> %t" eff (Params.print_region_param region)
