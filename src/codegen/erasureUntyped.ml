let rec typed_to_untyped_exp sub (typed_exp : Typed.expression) :
    UntypedSyntax.expression =
  let res = typed_to_untyped_exp_sub sub typed_exp in
  { it = res; at = Location.unknown }

and typed_to_untyped_exp_sub sub (typed_exp : Typed.expression) :
    UntypedSyntax.plain_expression =
  match typed_exp with
  | Typed.Var v -> UntypedSyntax.Var v
  (* | Typed.BuiltIn _ -> failwith __LOC__ *)
  | Typed.Const c -> UntypedSyntax.Const c
  | Typed.Tuple elist ->
      UntypedSyntax.Tuple (List.map (fun x -> typed_to_untyped_exp sub x) elist)
  | Typed.Record _ -> failwith __LOC__
  | Typed.Variant (l, e) ->
      UntypedSyntax.Variant (l, Some (typed_to_untyped_exp sub e))
  | Typed.Lambda abs ->
      UntypedSyntax.Lambda (typed_to_untyped_abs_with_ty sub abs)
      ...
      UntypedSyntax.Handler
        {
          effect_clauses =
            Assoc.kmap
              (fun ((e, effty), a2) -> (e, typed_to_untyped_abs_2 sub a2))
              effcs;
          value_clause = typed_to_untyped_abs_with_ty sub valc;
          finally_clause =
...
and typed_to_untyped_comp sub (typed_comp : Typed.computation) :
    UntypedSyntax.computation =
  let res = typed_to_untyped_comp_sub sub typed_comp in
  { it = res; at = Location.unknown }

and typed_to_untyped_comp_sub sub (typed_comp : Typed.computation) :
    UntypedSyntax.plain_computation =
  ...
  | Typed.Match (e, _resTy, alst) ->
      ...
      let alst' =
        List.map
          (fun (a : Typed.abstraction) -> typed_to_untyped_abs sub a)
          alst
      in
      UntypedSyntax.Match (e', alst')
  | Typed.Apply (e1, e2) ->
      UntypedSyntax.Apply
        (typed_to_untyped_exp sub e1, typed_to_untyped_exp sub e2)
  | Typed.Handle (e, c) ->
      UntypedSyntax.Handle
        (typed_to_untyped_exp sub e, typed_to_untyped_comp sub c)
  | Typed.Call (eff, e, absty) -> failwith __LOC__
  | Typed.Op (eff, e) -> failwith __LOC__
  | Typed.Bind (c1, (p, c2)) ->
      UntypedSyntax.Let
        ( [ (typed_to_untyped_pattern p, typed_to_untyped_comp sub c1) ],
...
and typed_to_untyped_abs_with_ty sub
    ((e_p, e_ty, e_c) : Typed.abstraction_with_ty) : UntypedSyntax.abstraction =
  (typed_to_untyped_pattern e_p, typed_to_untyped_comp sub e_c)

and typed_to_untyped_abs sub (abs : Typed.abstraction) :
    UntypedSyntax.abstraction =
  let (p : Typed.pattern), (c : Typed.computation) = abs in
  (typed_to_untyped_pattern p, typed_to_untyped_comp sub c)

and typed_to_untyped_abs_2 sub ((e_p1, e_p2, e_c) : Typed.abstraction2) :
    UntypedSyntax.abstraction2 =
...
and typed_to_untyped_pattern (pat : Typed.pattern) : UntypedSyntax.pattern =
  let res =
    ...
    | Typed.PTuple ps ->
        UntypedSyntax.PTuple (List.map (fun p -> typed_to_untyped_pattern p) ps)
