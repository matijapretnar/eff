let print_commands cmds =
  let print_command =
    if !Config.pure_print then PurePrint.print_command
    else SimplePrint.print_command
  in
  Print.sequence "\n\n;;\n\n" print_command cmds
...
let compile_file st filename =
  ...
  let separator =
    ( Sugared.Term
        (Sugared.Const (Const.of_string "End of pervasives"), Location.unknown)
    , Location.unknown )
  in
  ...
  let cmds =
    if !Config.disable_optimization then cmds
    else Optimize.optimize_commands cmds
  in
  ...
  ignore
    (Sys.command
       ("echo \"commit SHA: $(git rev-parse HEAD)\" >> " ^ compiled_file)) ;
  ...
  ignore (Sys.command ("echo ''" ^ " >> " ^ compiled_file)) ;
  ignore
    (Sys.command ("echo '=== END SOURCE ===\n*)'" ^ " >> " ^ compiled_file)) ;
  ignore (Sys.command ("echo ''" ^ " >> " ^ compiled_file)) ;
  ignore (Sys.command ("ocamlc " ^ temporary_file)) ;
  ignore
    (Sys.command
       ( "ocamlc -dsource -w -A " ^ temporary_file ^ " >>" ^ compiled_file
       ^ " 2>&1" ))
