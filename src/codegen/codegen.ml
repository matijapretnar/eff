...
and pretty_var x = string (Syntax.Variable.string_of (fst x))

and pretty_const c =
  match  c with
  | Syntax.Int k    -> string (string_of_int k)
  | Syntax.String s -> dquotes (string s)
  | Syntax.Bool b   -> string (string_of_bool b)

and pretty_abstraction (v, c) =
  pretty_var v ^+^ string "->" ^+^ prettyC c

and pretty_pure_abstraction (v, e) =
  pretty_var v ^+^ string "->*" ^+^ prettyE e

and pretty_h_effs cases  =  pretty_h_effs_aux (Syntax.EffectMap.bindings cases)
and pretty_h_effs_aux cases =
  match cases with
  | [] -> string "Nil"
  | (eff, (p, k, c)) :: cases ->
    string "Cons" ^+^ 
       parens ( dquotes (string (Syntax.Effect.string_of eff)) ^^ comma ^+^ parens (string "fun" ^+^ pretty_var p ^+^ pretty_var k ^+^ string "->" ^+^ prettyC c) ^^ comma ^+^ pretty_h_effs_aux cases)

and pretty_effect (eff,_,_) = dquotes (string (Syntax.Effect.string_of eff))

and pretty_handler h =
 braces (space ^^ prefix 3 1 (string "value = " ^^ parens (string "fun" ^+^ pretty_abstraction h.val_clause) ^^ semi) 
                             (string "effect_cases = " ^^ pretty_h_effs h.eff_clauses))      
