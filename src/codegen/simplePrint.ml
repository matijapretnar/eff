...
let rec print_expression ?max_level e ppf =
  ...
  | CastExp (e1, _) -> print "%t" (print_expression e1)
  ...
  | LambdaTyCoerVar (_, _, e1) -> print "%t " (print_expression e1)
  | LambdaDirtCoerVar (_, _, e1) -> print "%t " (print_expression e1)
...
and print_computation ?max_level c ppf =
  ...
  | LetVal (v, e1, c1) ->
      print "let %t = (Value %t) in %t" (print_variable v)
...
let print_command (cmd, _) ppf =
  ...
  | Typed.DefEffect (eff, (ty1, ty2)) as full_effect ->
      Print.print ppf "type (_, _) effect += %s : (%t, %t) effect" eff
