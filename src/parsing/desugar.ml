...
and handler loc ctx
  ...
  let rec construct_eff_clause (eff, eff_cs_lst) =
    ...
    | a2s ->
      ...
      let x_pvar = Untyped.add_loc (Untyped.PVar x) loc in
      let k = fresh_variable (Some "$continuation") in
      let k_var = Untyped.add_loc (Untyped.Var k) loc in
      let k_pvar = Untyped.add_loc (Untyped.PVar k) loc in
      let match_cs_constructor (p1, p2, t) =
        let vars1, p1 = pattern p1 in
        let vars2, p2 = pattern p2 in
        let t = computation (vars1 @ vars2 @ ctx) t in
        (Untyped.add_loc (Untyped.PTuple [p1; p2]) loc, t)
      in
      let match_cs = List.map match_cs_constructor a2s in
      let match_term = Untyped.add_loc (
          Untyped.Match ( Untyped.add_loc (Untyped.Tuple [x_var; k_var]) loc
                        , match_cs)) loc
      in
      (eff, (x_pvar, k_pvar, match_term))
  in
  let collected_eff_cs = List.fold_right group_eff_cs eff_cs [] in
  let constructed_eff_cs = List.map construct_eff_clause collected_eff_cs in
  ( []
  , { Untyped.effect_clauses= constructed_eff_cs
