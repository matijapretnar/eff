...
let fresh_ty_with_skel skel =
  ...
  (* α : τ₁ -> τ₂ *)
  | SkelArrow (sk1, sk2) ->
      let tvar1 = Coercion.fresh_ty_with_skel sk1
      and dtvar2 = Coercion.fresh_dirty_with_skel sk2 in
      Type.arrow (tvar1, dtvar2)
  (* α : τ₁ x τ₂ ... *)
  | SkelTuple sks ->
      let tvars = List.map Coercion.fresh_ty_with_skel sks in
      Type.tuple tvars
  (* α : ty_name (τ₁, τ₂, ...) *)
  ...
  (* α : τ₁ => τ₂ *)
  | SkelHandler (sk1, sk2) ->
      let dtvar1 = Coercion.fresh_dirty_with_skel sk1
      and dtvar2 = Coercion.fresh_dirty_with_skel sk2 in
      Type.handler (dtvar1, dtvar2)

let process_skeleton_parameter_equality sub (paused : Coercion.resolved)
    rest_queue sp1 sk2a =
  let k = sp1 in
  let v = sk2a in
  ...
  let cons_subbed =
    Substitution.apply_substitutions_to_constraints sub1
      (Coercion.return_resolved paused rest_queue)
  in
  ( Substitution.add_skel_param_substitution k v sub,
    Coercion.empty_resolved,
    cons_subbed )

let skel_eq_step sub (paused : Coercion.resolved) rest_queue sk1 sk2 =
  match (sk1, sk2) with
  (* ς = ς *)
  ...
  | SkelArrow (ska, skb), SkelArrow (skc, skd) ->
      ( sub,
        paused,
        Coercion.add_list_to_constraints
          [ Coercion.SkelEq (ska, skc); Coercion.SkelEq (skb, skd) ]
          rest_queue )
  (* τ₁₁ => τ₁₂ = τ₂₁ => τ₂₂ *)
  ...
  | SkelApply (ty_name1, sks1), SkelApply (ty_name2, sks2)
    when ty_name1 = ty_name2 && List.length sks1 = List.length sks2 ->
      ( sub,
        paused,
        Coercion.add_list_to_constraints
          (List.map2 (fun sk1 sk2 -> Coercion.SkelEq (sk1, sk2)) sks1 sks2)
          rest_queue )
...
and ty_omega_step sub (paused : Coercion.resolved) cons rest_queue omega =
  function
  (* ω : A <= A *)
  ...
  | { term = Type.Arrow (a1, dirty1); _ }, { term = Type.Arrow (a2, dirty2); _ }
    ->
      let new_ty_coercion_var_coer, ty_cons = Coercion.fresh_ty_coer (a2, a1)
      and dirty_coercion_c, dirty_cnstrs =
        Coercion.fresh_dirty_coer (dirty1, dirty2)
      in
      let k = omega in
      let v =
      ...
      ( Substitution.add_type_coercion k v sub,
        paused,
        Coercion.add_to_constraints ty_cons (rest_queue @ dirty_cnstrs) )
  (* ω : A₁ x A₂ x ... <= B₁ x B₂ x ...  *)
  | { term = Type.Tuple tys; _ }, { term = Type.Tuple tys'; _ }
    when List.length tys = List.length tys' ->
      let coercions, conss =
        List.fold_right2
          (fun ty ty' (coercions, conss) ->
            let coercion, ty_cons = Coercion.fresh_ty_coer (ty, ty') in
      ...
      ( Substitution.add_type_coercion k v sub,
        paused,
        Coercion.add_list_to_constraints conss rest_queue )
  (* ω : ty (A₁,  A₂,  ...) <= ty (B₁,  B₂,  ...) *)
  ...
  | ( { term = Type.Handler (drty11, drty12); _ },
      ...
      let drty_coer1, dirty_cons1 = Coercion.fresh_dirty_coer (drty21, drty11)
      and drty_coer2, dirty_cons2 =
        Coercion.fresh_dirty_coer (drty12, drty22)
      in
      let k = omega in
  ...
  | ( { term = Type.TyParam p1; ty = SkelParam s1 },
    ...
    when s1 = s2 ->
      ...
      (sub, Coercion.resolve_ty_constraint paused omega p1 p2 s1, rest_queue)
  ...
  | a, { term = Type.TyParam _; ty = SkelParam _ as skel_tv } ->
      ...
      ( sub,
        paused,
        Coercion.add_to_constraints
          (Coercion.SkelEq (skel_tv, skel_a))
          (Coercion.add_to_constraints cons rest_queue) )
  ...
  | _, { term = Type.TyParam tv; ty = skel } ->
      ...
      let cons_subbed =
        Substitution.apply_substitutions_to_constraints sub1
          (Coercion.return_resolved paused (cons :: rest_queue))
      in
...
and dirt_omega_step sub paused rest_queue omega dcons =
  ...
  | ( { effect_set = s1; row = ParamRow v1 },
      ...
      if Type.EffectSet.is_empty s1 then
        (sub, Coercion.resolve_dirt_constraint paused omega dcons, rest_queue)
      else
        ...
        in
        let new_cons = Coercion.DirtOmega (omega', omega_ty') in
        ( Substitution.merge sub sub',
          Coercion.empty_resolved,
          Substitution.apply_substitutions_to_constraints sub'
            (Coercion.return_resolved paused rest_queue
            |> Coercion.add_to_constraints new_cons) )
  (* ω : Ø <= Δ₂ *)
  ...
  | { effect_set = s1; row = EmptyRow }, { effect_set = s2; row = ParamRow v2 }
    ->
      ...
      ( Substitution.merge sub sub1,
        ...
        Substitution.apply_substitutions_to_constraints sub1
          (Coercion.return_resolved paused rest_queue) )
  | _ -> (sub, Coercion.resolve_dirt_constraint paused omega dcons, rest_queue)

let rec unify (sub, paused, queue) =
  (* Print.debug "SUB: %t" (Substitution.print_substitutions sub); *)
  (* Print.debug "PAUSED: %t" (Coercion.print_constraints paused); *)
  (* Print.debug "QUEUE: %t" (Coercion.print_constraints queue); *)
  match queue with
  | [] -> (sub, paused)
  | cons :: rest_queue ->
      let new_state =
        match cons with
        (* τ₁ = τ₂ *)
        | Coercion.SkelEq (sk1, sk2) ->
            skel_eq_step sub paused rest_queue sk1 sk2
        (* ω : A <= B *)
        | Coercion.TyOmega (omega, tycons) ->
            ty_omega_step sub paused cons rest_queue omega tycons
        (* ω : Δ₁ <= Δ₂ *)
        | Coercion.DirtOmega (omega, dcons) ->
            dirt_omega_step sub paused rest_queue omega dcons
      in
      unify new_state

let solve constraints =
  (* Print.debug "constraints: %t" (Coercion.print_constraints constraints); *)
  let solved =
    unify (Substitution.empty, Coercion.empty_resolved, constraints)
  in
  (* Print.debug "sub: %t" (Substitution.print_substitutions sub); *)
  (* Print.debug "solved: %t" (Coercion.print_constraints solved); *)
  solved
