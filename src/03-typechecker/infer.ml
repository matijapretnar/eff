...
and infer_pattern' ~loc state = function
  ...
  | Untyped.PRecord flds ->
      ...
      ( Pattern.pRecord out_ty (Type.Field.Map.of_bindings flds'),
...
and infer_abstraction state (pat, cmp) : Term.abstraction * Constraint.t =
  ...
  let state' = extend_vars (Pattern.bound_vars trgPat) state in
  ...
  let state' =
    extend_vars
      (Variable.Map.compatible_union
         (Pattern.bound_vars trgPat1)
         (Pattern.bound_vars trgPat2))
      state
  in
...
let process_top_let ~loc state defs =
  let fold (pat, cmp) (state', defs) =
    ...
    in
    let vars' =
      Variable.Map.map (Substitution.apply_sub_ty sub) (Pattern.bound_vars pat')
    in
    let params =
      match cmp.it with
      | Parser.UntypedSyntax.Value _ ->
