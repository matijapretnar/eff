    ...
    match c with
      ...
      | I.Match (e, lst) ->
          ...
          in
            ...
            check_pats (List.map fst lst) tctx ;
            t_out
...
let rec is_irrefutable p tctx =
  match fst p with
    | P.Var _ -> true
    | P.As (p',_) -> is_irrefutable p' tctx
    | P.Tuple lst -> List.for_all (fun x -> is_irrefutable x tctx) lst
    | P.Record lst -> List.for_all (fun (_, p') -> is_irrefutable p' tctx) lst
    | P.Variant (lbl, op) ->
        let single_tag =
          match Ctx.find_variant lbl tctx with
            | Some v ->
