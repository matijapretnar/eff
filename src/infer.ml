  ...
  let rec infer (p, pos) =
    match p with
      ...
      | Pattern.Record (((fld, _) :: _) as lst) ->
          (match Tctx.infer_field !Tctx.global fld with
      ...
      | Pattern.Variant (lbl, p) ->
          (match Tctx.infer_variant !Tctx.global lbl with
...
and infer_expr ctx cstr (e,pos) =
  match e with
    ...
    | Core.Operation (e, op) ->
      (match Tctx.infer_operation !Tctx.global op with
  ...
  let rec infer ctx (c, pos) =
    match c with
      ...
      | Core.Match (e, lst) ->
          ...
          Exhaust.check_patterns ~pos (List.map fst lst);
          t_out
