...
plain_topdef:
  ...
  | EFFECT eff = effect_sym COLON t1 = prod_ty ARROW t2 = ty
    { Commands.DefEffect (eff, (t1, t2))}
  | EFFECT eff = effect_sym COLON t = prod_ty
...
plain_binop_term:
  ...
  | t1 = binop_term CONS t2 = binop_term
    { let tuple = {it= Tuple [t1; t2]; at= Location.make $startpos $endpos} in
      Variant (Type.cons_annot, Some tuple) }
  | t = plain_uminus_term
    { t }
...
plain_simple_term:
  ...
  | cst = const_term
    { Const cst }
  | PERFORM LPAREN eff = effect_sym t = term RPAREN
    { Effect (eff, t)}
  | PERFORM eff = effect_sym
...
match_case:
  | p = pattern ARROW t = term
    { Val_match (p, t) }
  | EFFECT LPAREN eff = effect_sym p = simple_pattern RPAREN k = simple_pattern ARROW t = term
    { Eff_match (eff, (p, k, t)) }
  | EFFECT eff = effect_sym k = simple_pattern ARROW t = term
    { let unit_loc = Location.make $startpos(eff) $endpos(eff) in
...
plain_cons_pattern:
  ...
  | p1 = variant_pattern CONS p2 = cons_pattern
    { let ptuple = {it= PTuple [p1; p2]; at= Location.make $startpos $endpos} in
      PVariant (Type.cons_annot, Some ptuple) }
