...
let ty_param_has_skel_step sub paused cons rest_queue tvar skel =
  ...
  | PrimSkel ps ->
      ...
      (Substitution.add_type_substitution tvar (Types.PrimTy ps) sub, [], Substitution.apply_substitutions_to_constraints sub1 (Typed.add_list_to_constraints paused rest_queue))
  (* α : τ₁ -> τ₂ *)
  | SkelArrow (sk1, sk2) ->
      ...
      let sub1 = Substitution.add_type_substitution_e k v in
      let cons_subbed = (Substitution.apply_substitutions_to_constraints sub1 (Typed.add_list_to_constraints paused rest_queue)) in
  ...
  | SkelApply (ty_name, sks) ->
      ...
      let sub1 = Substitution.add_type_substitution_e k v in
      let cons_subbed = Substitution.apply_substitutions_to_constraints sub1 (add_list_to_constraints paused rest_queue) in
      (Substitution.add_type_substitution k v sub, [], add_list_to_constraints cons_subbed conss)
  (* α : τ₁ => τ₂ *)
  | SkelHandler (sk1, sk2) ->
      let tvar1, cons1 = Typed.fresh_ty_with_skel sk1
      and tvar2, cons2 = Typed.fresh_ty_with_skel sk2
      ...
      let sub1 = Substitution.add_type_substitution_e k v in
      let cons_subbed = Substitution.apply_substitutions_to_constraints sub1 (Typed.add_list_to_constraints paused rest_queue)
      in
      ( Substitution.add_type_substitution k v sub
      , []
      , Typed.add_to_constraints cons1 cons_subbed |> Typed.add_to_constraints cons2 )
...
and ty_omega_step sub paused cons rest_queue omega =
  function
    ...
    | Types.Arrow (a1, dirty1), Types.Arrow (a2, dirty2) ->
        ...
        in
        (Substitution.add_type_coercion k v sub, paused, add_to_constraints ty_cons rest_queue |> add_to_constraints dirty_cons)
    (* ω : A₁ x A₂ x ... <= B₁ x B₂ x ...  *)
    ...
    | Types.Handler (drty11, drty12), Types.Handler (drty21, drty22) ->
        ...
        in
        (Substitution.add_type_coercion k v sub, paused, add_to_constraints drty_cons1 rest_queue |> add_to_constraints drty_cons2)
    (* ω : α <= A /  ω : A <= α *)
    | Types.TyParam tv, a
     |a, Types.TyParam tv ->
...
and dirt_omega_step sub paused cons rest_queue omega dcons =
  ...
  | {effect_set= s1; row= ParamRow v1}, {effect_set= s2; row= ParamRow v2} ->
      if Types.EffectSet.is_empty s1 then (sub, cons :: paused, rest_queue)
        ...
        in
        (Substitution.merge sub sub', [], Substitution.apply_substitutions_to_constraints sub' (add_list_to_constraints paused rest_queue |> add_to_constraints new_cons))
  (* ω : Ø <= Δ₂ *)
  ...
  | {effect_set= s1; row= ParamRow v1}, {effect_set= s2; row= EmptyRow}
    when Types.EffectSet.is_empty s1 && Types.EffectSet.is_empty s2 ->
      ...
      in
      (Substitution.merge sub sub1, [], Substitution.apply_substitutions_to_constraints sub1 (add_list_to_constraints paused rest_queue))
  (* ω : O₁ <= O₂ *)
