...
let infer_pattern st type_context pp =
  ...
  let rec infer st' vars {it= p; at= loc} =
    ...
    | Untyped.PConst const ->
        (T.Basic (Const.infer_ty const), st', vars)
    | Untyped.PTuple ps ->
        let (st', vars), pst =
          fold_map
            (fun (st'', vars') pa ->
...
and infer_let st type_context loc defs =
  ...
  let rec find_duplicate xs ys =
    ...
    | [] ->
        None
    | x :: xs ->
        if List.mem x ys then Some x else find_duplicate xs ys
  in
...
and infer_expr st type_context {it= e; at= loc} =
  let tty, st =
    match e with
    | Untyped.Var x ->
        (ctx_lookup ~loc st x, st)
    | Untyped.Const const ->
        (T.Basic (Const.infer_ty const), st)
