...
module Backend : BackendSignature.T = struct
  ...
  let process_computation state c drty =
    ...
    Format.fprintf !Config.output_formatter "@[- : %t = %t@]@." (Types.print_target_dirty drty)
      (Value.print_value v);
    state

  let process_type_of state c drty =
    Format.fprintf !Config.output_formatter "@[- : %t@]@."     (Types.print_target_dirty drty);
    state
  ...
  let process_top_let state defs vars =
    let state' =
      List.fold_right
        (fun (p, c) st -> let v = Eval.run st c in Eval.extend p v st )
        defs state
    in
    List.iter
      (fun (x, tysch) ->
