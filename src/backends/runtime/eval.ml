...
let rec extend_value p v state =
  ...
  | ExEff.PTuple ps, Value.Tuple vs -> List.fold_right2 extend_value ps vs state
  ...
  | ExEff.PVariant (lbl, p), Value.Variant (lbl', Some v) when lbl = lbl' ->
...
let rec ceval state c =
  ...
  | ExEff.LetRec (defs, c) ->
      ...
      let state =
        extend_let_rec state (Assoc.of_list (List.map converter defs))
      in
      ceval state c
  ...
  | ExEff.Bind (c, a) -> (
      ...
      | V.Call (eff, v, clo) ->
          sequence (eval_closure state a) (V.Call (eff, v, clo)))
...
and eval_handler state
    { ExEff.effect_clauses = ops; ExEff.value_clause = pat, _, com } =
  let eval_op a2 =
    let p, kvar, c = a2 in
