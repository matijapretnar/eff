...
let abstraction_is_id (p, c) =
  ...
  match p with
  | Multicore.PVar v -> CoreTypes.Variable.fold (fun desc _ -> desc = "$id_par") v
  | _ -> false
...
and of_expression {it; at} =
  ...
  | CoreSyntax.Annotated (e, ty) -> Multicore.Annotated (of_expression e, of_type ty)
  ...
  | CoreSyntax.Handler {effect_clauses; value_clause; finally_clause} ->
      ...
      if abstraction_is_id finally_clause_abs then
        ...
        Multicore.Lambda
          (Multicore.PVar ghost_bind, Multicore.Apply (Multicore.Lambda finally_clause_abs, match_handler))
...
and of_computation {it; at} =
  ...
  | CoreSyntax.Apply (e1, e2) -> Multicore.Apply (of_expression e1, of_expression e2)
  | CoreSyntax.Check c -> Multicore.Check (of_computation c)
  | CoreSyntax.Handle (e, c) ->
      (* Non-trivial case *)
      let modified_handler = of_expression e in
      let thunked_c = Multicore.Lambda (Multicore.PNonbinding, of_computation c) in
      Multicore.Apply (modified_handler, thunked_c)

and of_pattern {it; at} =
  match it with
  | CoreSyntax.PVar var -> Multicore.PVar var
  | CoreSyntax.PAnnotated (p, ty) -> Multicore.PAnnotated (of_pattern p, of_type ty)
