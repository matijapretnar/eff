...
(* ------------------------------------------------------------------------ *)
  ...
  type state = {prog: OcamlSyntax.cmd list; typing: ExplicitInfer.state}
  ...
  let process_computation state c ty =
    let ex_st', {ExplicitInfer.computation = exeff_c} = ExplicitInfer.type_computation state.typing c in
  ...
  let process_def_effect state (eff, (ty1, ty2)) =
    update state (OcamlSyntax.DefEffect (eff, (process_type ty1, process_type ty2)))
