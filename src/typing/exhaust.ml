...
let rec remove_as = function
  | P.PAs ((p', _), _) -> remove_as p'
  | p -> p

(* Reads constructor description from a pattern, discarding any [P.PAs] layers. *)
let rec cons_of_pattern p =
  match fst p with
  | P.PAs (p, _) -> cons_of_pattern p
  | P.PTuple lst -> Tuple (List.length lst)
  | P.PRecord [] -> assert false
  | P.PRecord ((lbl, _) :: _) ->
  ...
  | P.PVariant (lbl, opt) -> Variant (lbl, opt <> None)
  | P.PConst c -> Const c
  | P.PVar _ | P.PNonbinding -> Wildcard
...
let pattern_of_cons ~loc c lst =
  let plain = match c with
    | Tuple n -> P.PTuple lst
    | Record flds -> P.PRecord (List.combine flds lst)
    | Const const -> P.PConst const
    | Variant (lbl, opt) -> P.PVariant (lbl, if opt then Some (List.hd lst) else None)
    | Wildcard -> P.PNonbinding
  in
  (plain, loc)
...
let specialize_vector ~loc con = function
  ...
  | (p1, _) :: lst ->
    begin match con, remove_as p1 with
      | Tuple _, P.PTuple l -> Some (l @ lst)
      | Record all, P.PRecord def ->
        let get_pattern defs lbl = match C.lookup lbl defs with
          | Some p' -> p'
          | None -> (P.PNonbinding, loc)
        in
        Some ((List.map (get_pattern def) all) @ lst)
      | Variant (lbl, _), P.PVariant (lbl', opt) when lbl = lbl' ->
        begin match opt with
          | Some p -> Some (p :: lst)
          | None -> Some lst
        end
      | Const c, P.PConst c' when Const.equal c c' -> Some lst
      | _, (P.PNonbinding | P.PVar _) -> Some ((C.repeat (P.PNonbinding, loc) (arity con)) @ lst)
      | _, _ -> None
    end
...
let rec default = function
  ...
  | ((p,_) :: ps) :: lst ->
    begin match remove_as p with
      | (P.PNonbinding | P.PVar _) -> ps :: (default lst)
      | _ -> default lst
    end
  ...
  | n ->
    ...
    if present <> [] && missing = [] then
      ...
      | Some lst ->
        ...
        Some ((pattern_of_cons ~loc c (C.repeat (P.PNonbinding, loc) (arity c))) :: lst)


let rec old_of_new_pattern p =
  let old_p =
    match p.Untyped.term with
    | Untyped.PAs (p, x) -> P.PAs (old_of_new_pattern p, x)
    | Untyped.PTuple lst -> P.PTuple (List.map old_of_new_pattern lst)
    | Untyped.PRecord lst -> P.PRecord (Common.assoc_map old_of_new_pattern lst)
    | Untyped.PVariant (lbl, opt) -> P.PVariant (lbl, Common.option_map old_of_new_pattern opt)
    | Untyped.PConst c -> P.PConst c
    | Untyped.PVar x -> P.PVar x
    | Untyped.PNonbinding -> P.PNonbinding
  in
...
let rec new_of_old_pattern p =
  let new_p =
    match fst p with
    | P.PAs (p, x) -> Untyped.PAs (new_of_old_pattern p, x)
    | P.PTuple lst -> Untyped.PTuple (List.map new_of_old_pattern lst)
    | P.PRecord lst -> Untyped.PRecord (Common.assoc_map new_of_old_pattern lst)
    | P.PVariant (lbl, opt) -> Untyped.PVariant (lbl, Common.option_map new_of_old_pattern opt)
    | P.PConst c -> Untyped.PConst c
    | P.PVar x -> Untyped.PVar x
    | P.PNonbinding -> Untyped.PNonbinding
  in
  {
    Untyped.term = new_p;
