...
let extend ctx x ty_scheme = {ctx with variables = (x, ty_scheme) :: ctx.variables}
...
let infer_effect env eff =
  try
    Some (CoreSyntax.EffectMap.find eff env.effects)
  with
    | Not_found -> None

let add_effect env eff (ty1, ty2) =
  match infer_effect env eff with
  | None -> {env with effects = CoreSyntax.EffectMap.add eff (ty1, ty2) env.effects}
  | Some _ -> Error.typing ~loc:Location.unknown "Effect %s already defined." eff
