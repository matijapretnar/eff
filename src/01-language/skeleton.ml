...
let rec equal skel1 skel2 =
  ...
  | ( Apply { ty_name = ty_name1; skel_args = tys1 },
      ...
      let is_eq =
        ty_name1 = ty_name2
        && Print.debug "Skel eq part" = ()
        && TyParam.Map.equal (fun (t1, _) (t2, _) -> equal t1 t2) tys1 tys2
      in
      Print.debug "fst";
      TyParam.Map.iter (fun tp _ -> Print.debug "%t " (TyParam.print tp)) tys1;
      Print.debug "snd";
      TyParam.Map.iter (fun tp _ -> Print.debug "%t " (TyParam.print tp)) tys2;
      Print.debug "Iss eq: %b" is_eq;
      is_eq
